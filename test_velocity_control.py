...
# Phase 3: Forward flight (vx = 2 m/s for 5 seconds = 1m forward)
print("\n[3] Forward (vx = 0.5 m/s for 2 seconds)...")
start = time.time()
while time.time() - start < 5:
    set_velocity_target(2, 0, 0)
    time.sleep(0.05)
x, y, z, vx, vy, vz = get_local_position()
print(f"    pos=({x:.2f}, {y:.2f}, {z:.2f})")
...
# Phase 5: Yaw rotation (rotate 90 degrees at 0.1 rad/s = ~1.8 seconds for 90deg)
print("\n[5] Yaw rotation (yaw_rate = 0.1 rad/s for 5 seconds)...")
start = time.time()
while time.time() - start < 3:
    set_velocity_target(0, 0, 0, yaw_rate=0.1)
    time.sleep(0.05)
